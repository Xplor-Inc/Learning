...
using XploringMe.Core.Models.Entities;

namespace XploringMe.SqlServer.Repositories;
    ...
    where T : Entity
{
    ...
    public Repository(IContext context, ILogger<Repository<T>> logger)
    {
        Context = context;
        Logger  = logger;
    ...
    public virtual Result<T> Create(T entity, long createdById)
    {
        ...
        try
        {
            ...
            if (entity is Auditable auditable && auditable.UniqueId == Guid.Empty)
                auditable.UniqueId = Guid.NewGuid();
    ...
    private List<string> HandleException(Exception ex)
    {
        ...
        if (ex.InnerException != null)
            loggerError += $"InnerException : {ex.InnerException.Message} \n {ex.InnerException.StackTrace}";

        Logger.LogError(loggerError);
        return returnError;
    }
